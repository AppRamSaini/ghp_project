...
class UpdateRentPropertyScreen extends StatefulWidget {
  ...
  @override
  State<UpdateRentPropertyScreen> createState() =>
      UpdateRentPropertyScreenState();
}

class UpdateRentPropertyScreenState extends State<UpdateRentPropertyScreen> {
  ...
  setData() {
    ...
    rentController.text =
        double.parse(widget.propertyLis.rentPerMonth.toString())
            .toInt()
            .toString();
    securityDepositController.text =
        double.parse(widget.propertyLis.securityDeposit.toString())
            .toInt()
            .toString();
  ...
  Widget build(BuildContext context) {
    return BlocListener<UpdateRentPropertyCubit, UpdateRentPropertyState>(
      listener: (context, state) async {
        if (state is UpdateRentPropertyLoading) {
          showLoadingDialog(context, (ctx) {
            dialogueContext = ctx;
          });
        } else if (state is UpdateRentPropertySuccessfully) {
          snackBar(context, 'Rent property updated successfully', Icons.done,
              AppTheme.guestColor);

          Navigator.of(dialogueContext!).pop();
          Navigator.pushReplacement(context,
              MaterialPageRoute(builder: (_) => const ManagePropertyScreen()));
        } else if (state is UpdateRentPropertyFailed) {
          snackBar(context, 'Failed to update Rent property', Icons.warning,
              AppTheme.redColor);

          Navigator.of(dialogueContext!).pop();
        } else if (state is UpdateRentPropertyInternetError) {
