  ...
  Future fetchGetNotificationSettingsAPI() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        var newGetNotificationSettings =
            (responseData['data']['notification_settings'] as List)
                .map((e) => NotificationSetting.fromJson(e))
                .toList();
        notificationSettingsList.clear();

        notificationSettingsList.addAll(newGetNotificationSettings);
        emit(GetNotificationSettingsLoaded(
            notificationSettings: notificationSettingsList));
      } else {
        emit(GetNotificationSettingsFailed(
            errorMessage:
                'Failed to load Notification settings')); // Handle failure response
      }
    } on SocketException {
      emit(GetNotificationSettingsInternetError(
          errorMessage: "No internet connection.")); // Handle network issues
    } catch (e) {
      emit(GetNotificationSettingsFailed(
          errorMessage:
              'Notification settings Not Found!')); // Handle general errors
    }
  }
}
